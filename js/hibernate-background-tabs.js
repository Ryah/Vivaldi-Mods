(function () {
    // ============================================================================================================
    // Hibernate Background Tabs with Status Bar or Tabbar Button plus Keyboard Shortcut Mod
    // URL:         https://forum.vivaldi.net/post/396130 + https://forum.vivaldi.net/topic/60172/hibernate-background-tabs-keyboard-shortcut-hotkey + https://forum.vivaldi.net/post/437867
    // Description: Adds a status bar or tabbar button and keyboard shortcut that will hibernate all tabs except the active one
    // Author(s):   @nomadic   [ keyboard shortcut based on a mod by @LonM "Custom Keyboard Shortcuts Mod" ], fork by dude99
    ...
    function hibernateBackgroundTabsCombined() {
      ...
      function addHibernateButton() {
        ...
        if (!document.getElementById("hibernate")) {
          ...
          let elBefore = document.querySelector(".sync-and-trash-container > .button-toolbar:first-child");
          ...
          document.querySelector(".sync-and-trash-container").insertBefore(div, elBefore);
      ...
      let oldTabState = browser.classList.value.match(/\stabs\-[^\s]*/)[0];
      let browserObserver = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
          if (mutation.attributeName == "class") {
            ...
            // CHANGE: Check for changes in tab bar toggling or tab bar position and re-add if necessary
            let newTabState = mutation.target.classList.value.match(/\stabs\-[^\s]*/)[0];
            if (oldTabState !== newTabState) {
              oldTabState = newTabState;
              if (oldTabState !== " tabs-off") {
                addHibernateButton();			 
              }
            }
